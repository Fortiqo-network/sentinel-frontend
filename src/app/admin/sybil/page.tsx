    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {clusters.map((c) => (
            <div key={`${c.reason}-${c.key}`} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-4 py-3">
                ...
                <span className="text-xs font-medium text-slate-500">{c.size} accounts</span>
              </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {c.accounts.map((a) => (
                    <tr key={a.id}>
                      ...
                      <td className="px-4 py-2 text-slate-500">{a.display_name ?? "—"}</td>
                      <td className="px-4 py-2 font-mono text-xs text-slate-500">{a.signup_ip ?? "—"}</td>
