      ...
      {status === "error" ? (
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-sen-danger">
        ...
        <>
            ...
            {stats.map(({ label, value }) => (
              <Card key={label}>
                <CardContent className="pt-6">
                  <div
                    className={cn(
                      "text-2xl font-bold font-mono text-sen-text",
                      status === "loading" && "text-sen-muted",
                    )}
                  >
                    {value}
                  </div>
          ...
          <Card>
            ...
            <CardContent>
              {status === "loading" ? (
                <div className="flex h-[300px] items-center justify-center rounded-lg bg-sen-surface-2 text-sm text-sen-muted">
                  Loading usage…
                </div>
              ) : (
          ...
          <Card>
            ...
            <CardContent>
              {status === "loading" ? (
                <p className="text-sm text-sen-muted">Loading…</p>
              ) : !hasUsage || summary === null ? (
                <p className="text-sm text-sen-muted">
                ...
                <ul className="divide-y divide-sen-border">
                  {summary.breakdown.map((row) => (
                    <li key={row.key} className="flex items-center justify-between py-3">
                      <div>
                        <div className="text-sm font-medium text-sen-text">{row.label}</div>
                        <div className="text-xs text-sen-muted">
                          {row.invocations.toLocaleString("en-IN")}{" "}
                          {row.invocations === 1 ? "invocation" : "invocations"}
                        </div>
                      </div>
                      <div className="text-sm font-semibold font-mono text-sen-text">
