...
| Verify **report/event signing** | **Real (Ed25519)** — `core/signing.py` signs the canonical TrustReport (stored on the row) and the `verification.completed` event (`X-Sentinel-Ed25519` + `X-Sentinel-Public-Key` headers) so consumers can verify authenticity; ships unsigned (disabled) until `REPORT_SIGNING_KEY` is set. |
