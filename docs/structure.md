...
| GET/POST | `/v1/seller/kyc` | yes | **Seller KYC + payout destination** (identity + razorpay bank/UPI or stripe rail; account number encrypted at rest, masked to last-4) — submit / re-submit for admin review (2026-09-13) |
| GET/POST | `/v1/admin/kyc` · `/v1/admin/users/{id}/kyc` | admin | **KYC review queue** (masked) · audited full reveal (GET) · approve with the provider account ref → provisions billing `seller_payout_profiles` fail-closed, or reject with note (POST) (2026-09-13) |
...
- **Payout onboarding (2026-09-13):** `/seller/kyc` (identity + bank/UPI or Stripe rail, status + reviewer note) · `/admin/kyc` (queue → audited reveal → approve-with-account-ref / reject).
...
| **Seller KYC → payout profile** | **Real (2026-09-13)** — `/seller/kyc` → core-api `POST /v1/users/kyc` (bank number Fernet-sealed, last-4 kept) → `/admin/kyc` reveal (audited `admin.kyc_viewed`) → approve requires `fund_account_id`/`stripe_account_id` and calls billing `PUT /v1/payouts/profile` (`kyc_verified=true`) **before** `kyc_status=verified` (billing down → 502, unchanged); reject revokes best-effort. **Still owner-gated:** `PAYOUT_EXECUTION_ENABLED` test-mode run + RazorpayX fund-account creation (admin pastes the `fa_…` id today). |
...
| **Self-hosted agents — seller upstream API key** | **Real** — a seller registers a self-hosted endpoint and (optionally) the credential Sentinel presents when calling it: `upstream_auth = {header, key}`, **Fernet-encrypted at rest** (`agents.upstream_auth_enc`, migration `0025`; key = `UPSTREAM_SECRETS_KEY`, derived from `JWT_SECRET` if unset). **Never echoed** — read endpoints expose only `upstream_auth_set`; the plaintext egresses ONLY on the internal, token-gated `GET /internal/agents/{id}/invoke-config` (gateway injects the header when proxying) and `.../verify-config` (sentinel-verify probes an auth-protected endpoint pre-live; the key is never logged/placed in a finding). Endpoint URLs are SSRF-validated at write time (unresolvable hosts tolerated — verify is the reachability gate). |
| **x402 payout registration** | **Real (registration + charge); payout deferred** — a seller enables x402, sets a price, and gives a `pay_to` EVM address: `x402 = {price_usd, pay_to}` → stored `x402_config = {enabled, price_units, pay_to, network:base, asset:USDC, verified}` (migration `0025`) and the price is mirrored into `access_config.price_per_call_units` so the gateway charges it. Buyer pays credits (existing 3% `split_charge`, fee off the seller). **On-chain USDC payout to `pay_to` via a hosted, pluggable facilitator (batched) is the next milestone** (billing `seller_payout_profiles` gets an x402 rail). |
| **Programmatic (curl) registration** | **Real** — `POST /v1/agents/register` at the gateway (API key or session) → core-api internal `POST /internal/agents/register` with the **server-resolved** `owner_id` (never client-supplied), optional `auto_submit` chains submit-for-verification. One curl call takes a self-hosted/x402 agent from nothing into the verification pipeline. |
| **Seller usage / earnings** | **Real** — `GET /v1/agents/{id}/usage` (owner-only) returns the billing-ledger rollup (calls, gross, your earnings, platform fee) for one of your agents, windowable via `since`. |
...
| Promo codes (free listing / credit top-up) | **Real** — **DB-managed** (`promo_codes` table, migration `0024`). Admins create/disable/expire/cap codes at runtime via `/v1/admin/promo` (no redeploy); redeem via `POST /v1/promo/redeem` — validates active/expiry/global-cap (row-locked), grants credits through billing (idempotent), **single-use per user** (`promo_redemptions.uq_promo_redemption_user_code`, migration `0006`). `PROMO` env (JSON `{CODE: usd}`) is an optional first-boot **seed** only. Free-listing codes (`PROMO_FREE_LISTING_CODES`, default `["SENTINEL1"]`) waive the $10 create fee — a separate config perk. |
...
| **Audit log** (append-only `audit_events`) | **Real** — records agent retire / listing-paid / access block & unblock (actor, action, entity, details, time); money movements audited via the billing ledger. Migration `0004`. Admins query it at `GET /v1/admin/audit` (action/entity/actor filters + pagination) and export the filtered trail as CSV at `GET /v1/admin/audit/export` (row cap from `audit_export_max_rows`; spreadsheet-formula injection neutralised; the export is itself audited as `admin.audit.exported`). **Not tamper-evident** — append-only by convention only, no hash chain (E4a); money actions are not in the CSV (E4b). |
| **2FA** for developer & user accounts | **Planned** (`users.mfa_enabled` column exists; TOTP/WebAuthn enrolment + challenge not built) |
| Verify static lane: SAST (Semgrep+Bandit), **agent-safety rules** (model-output→exec, obfuscated exec, env-dump egress, tool-returns-env), **undeclared-egress** (source URLs vs manifest `egress_allow`), supply-chain (CVE), **secrets**, **code-quality** (radon), **AI review** (Gemini), scoring/tiers (rubric v1.1) | **Real** — undeclared-egress is `medium` (disclosure-only, no score impact); agent-safety Semgrep rules take effect once the sandbox image is rebuilt; code-quality/AI-review **defer** cleanly when there's no source or no Gemini key |
