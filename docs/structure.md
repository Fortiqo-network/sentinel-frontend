...
| GET/POST | `/v1/admin/overview` · `/v1/admin/agents/enable-all` · `/v1/admin/agents/{id}/publish` · `/v1/admin/developers/{id}/settle` | admin | **Admin console** (role-gated): platform counts, enable agents → live, settle a developer's accrued earnings |
...
| **Developer settlement** (admin-gated) | **Real (backend)** — earnings accrue to `developer:{id}:payable` (pending); admin **settles** → moves to `developer:{id}:available` (withdrawable, `DEVELOPER_SETTLEMENT` ledger type); payouts draw from `available` only. core-api admin role (`UserRole.ADMIN` in `roles`) + `get_admin_user` gate; `/v1/admin/*` via gateway. **Admin dashboard UI + analytics pending.** |
| Admin: **enable all agents → live** | **Real (backend)** — `POST /v1/admin/agents/enable-all` (+ per-agent `/publish`) sets non-live agents live + health active; the 6h sweep then re-checks endpoints. |
...
| Verify **conformance + reliability** stage | **Real** — probes the agent's live endpoint: `/health` 2xx + `/invoke` honours the canonical `{task_id, input}` contract (valid JSON), sampled K times for a reliability success rate. Findings are advisory (bounded ≤0.10 penalty); defers honestly when no endpoint. |
